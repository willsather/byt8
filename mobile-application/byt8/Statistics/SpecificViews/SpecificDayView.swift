...
struct SpecificDayView: View {
    
    @Environment(\.colorScheme) var colorScheme: ColorScheme
    @EnvironmentObject var statsInfo: StatsInfo
    @EnvironmentObject var specificData: SpecificData
    ...
    var body: some View {
        ...
        RefreshableScrollView(content: {
            ...
            VStack {
                ...
                SpecificStatsListView(color: self.color, dayType: self.dayType, specificStats: self.specificData)
                    .padding(.top)
            }
        ...
        onRefresh: { control in
            ...
            self.specificData.getSpecificData(dayType: self.dayType)
            
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
                
                self.specificDays = self.statsInfo.getSpecificDay(type: self.dayType)
